...
export class PronunciationService {
    ...
    try {
      ...
      if (this.isSpeechRecognitionAvailable() && audioBlob) {
        try {
          ...
          const recognitionPromise = new Promise<TranscriptionResult>(
            (resolve) => {
              ...
              recognition.onend = async () => {
                ...
                if (!finalTranscript) {
                  console.log("No speech detected, trying server API...");

                  // Try with the server API
                  try {
                    const serverResult = await this.fallbackToServerAPI(
                      audioBlob,
                      referenceText,
                      language
                    );
                ...
                if (this.useRealisticTranscription && referenceText) {
              ...
              try {
                ...
                recognition.start();
                console.log("Speech recognition started, playing audio...");
          ...
          console.error("Error with audio playback approach:", playbackError);
  ...
  private async createRealisticTranscription(
    ...
    try {
      ...
      const response = await fetch("/api/ai-proxy/realistic-transcription", {
        method: "POST",
        headers: {
  ...
  private findSimilarWord(word: string, referenceWords: string[]): string | null {
    ...
    const candidates = referenceWords.filter(
      (refWord) =>
        refWord[0]?.toLowerCase() === word[0]?.toLowerCase() && refWord !== word
    );
