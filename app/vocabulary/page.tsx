...
import { Search, Filter, BookOpen, Clock, Plus, Bookmark, TrendingUp } from "lucide-react"
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-game-accent/80">
            Mở rộng vốn từ vựng tiếng Anh của bạn thông qua các phương pháp học hiệu quả
          </p>
        </div>
        ...
        <Tabs defaultValue="learning" onValueChange={setActiveTab} className="w-full">
          <TabsList className="mb-6 grid w-full grid-cols-3 bg-game-background">
            ...
            <TabsTrigger value="explore" className="data-[state=active]:bg-white data-[state=active]:text-game-primary">
              Khám phá
            </TabsTrigger>
            <TabsTrigger
              value="bookmarked"
          ...
          <TabsContent value="learning" className="space-y-6">
            {learningProgress.length > 0 ? (
                  ...
                  >
                    <Card className="game-card h-full transition-all hover:border-game-primary/50">
                      <CardHeader className="pb-2">
                        <CardTitle className="text-xl text-game-accent">{course.title}</CardTitle>
                        <CardDescription className="flex items-center gap-1 text-game-accent/70">
                          <Clock className="h-4 w-4" /> {course.lastLearned}
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="pb-2">
                        <div className="mb-4 space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="text-game-primary">{course.progress}%</span>
                          </div>
                          <Progress
                        ...
                        <div className="flex flex-wrap gap-2">
                          <Badge variant="outline" className="bg-game-primary/10 text-game-primary">
                            {course.dueWords} từ cần ôn tập
                          </Badge>
                          {course.streak > 0 && (
                            <Badge variant="outline" className="bg-amber-100 text-amber-700">
                              <TrendingUp className="mr-1 h-3 w-3" /> {course.streak} ngày liên tục
                            </Badge>
                          )}
                        </div>
                      </CardContent>
              ...
              <div className="flex flex-col items-center justify-center rounded-lg bg-white p-8 text-center">
                ...
                <h3 className="text-xl font-bold text-game-accent">Chưa có khóa học nào</h3>
                <p className="mb-4 text-game-accent/70">Bạn chưa bắt đầu học bất kỳ bộ từ vựng nào</p>
                <Button className="game-button" onClick={() => setActiveTab("explore")}>
          ...
          <TabsContent value="explore" className="space-y-6">
            <div className="mb-4">
              <h2 className="text-2xl font-bold text-game-accent">Khóa học nổi bật</h2>
              <p className="text-game-accent/70">Các bộ từ vựng được đề xuất cho bạn</p>
            </div>
                  ...
                  >
                    <Card className="game-card h-full transition-all hover:border-game-primary/50">
                      <CardHeader className="pb-2">
                        ...
                        <CardTitle className="mt-2 text-xl text-game-accent">{course.title}</CardTitle>
                        <CardDescription className="text-game-accent/70">{course.description}</CardDescription>
                      </CardHeader>
                      <CardContent className="pb-2">
                        <div className="flex flex-wrap gap-2">
                      ...
                      <CardFooter className="flex justify-between">
                        <Button variant="outline" className="text-game-primary">
                          Xem trước
                        </Button>
                        <Button
            ...
            <div className="mb-4 mt-8">
              <h2 className="text-2xl font-bold text-game-accent">Tất cả khóa học</h2>
              <p className="text-game-accent/70">Khám phá tất cả các bộ từ vựng có sẵn</p>
            </div>
          ...
          <TabsContent value="bookmarked" className="space-y-6">
            ...
            {filteredCourses.filter((course) => course.isBookmarked).length === 0 && (
              <div className="flex flex-col items-center justify-center rounded-lg bg-white p-8 text-center">
                <Bookmark className="mb-4 h-16 w-16 text-gray-300" />
                <h3 className="text-xl font-bold text-game-accent">Chưa có khóa học nào được lưu</h3>
                <p className="mb-4 text-game-accent/70">Bạn chưa lưu bất kỳ bộ từ vựng nào</p>
                <Button className="game-button" onClick={() => setActiveTab("explore")}>
                  Khám phá các khóa học
                </Button>
              </div>
        ...
        <VocabularyModeDialog open={showModeDialog} onOpenChange={setShowModeDialog} courseId={selectedCourseId} />
      </main>
    </div>
  )
}
