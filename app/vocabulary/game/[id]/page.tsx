...
import { ArrowLeft, Clock, Trophy, Check, RefreshCw, HelpCircle } from "lucide-react"
...
// Đặt từ vào bảng
const placeWord = (grid: string[][], word: string, startRow: number, startCol: number, direction: number[]) => {
  const [dRow, dCol] = direction
...
const createGridData = (words: { word: string; definition: string; found: boolean }[]) => {
...
export default function VocabularyGamePage({ params }: { params: { id: string } }) {
  ...
  const handleMouseUp = () => {
    ...
    if (wordIndex !== -1) {
      ...
      const alreadyFound = words.some((w) => w.word === currentWord && w.found)
  ...
  // Kết thúc trò chơi
  const handleFinish = () => {
    router.push(`/vocabulary/${gameData.courseId}`)
  }

  // Tính toán tiến độ
  const progress = (words.filter((word) => word.found).length / words.length) * 100
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-6 flex items-center justify-between">
          <Button
            ...
            onClick={() => router.push(`/vocabulary/${gameData.courseId}`)}
          >
          ...
          <div className="text-right">
            <h2 className="text-lg font-bold text-game-accent">{gameData.courseTitle}</h2>
            <p className="text-sm text-game-accent/70">{gameData.sectionTitle}</p>
          </div>
        </div>
        ...
        {!gameOver && !gameCompleted ? (
            ...
            <div className="mb-4 space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-game-accent">
                  Tìm thấy: {words.filter((word) => word.found).length}/{words.length} từ
                </span>
          ...
          >
            ...
            <div className="mb-6 grid w-full max-w-md grid-cols-3 gap-4">
              ...
              <div className="rounded-md bg-gray-50 p-4 text-center">
                <div className="text-2xl font-bold text-green-600">{words.filter((word) => word.found).length}</div>
                <div className="text-sm text-game-accent/70">Từ đã tìm</div>
              </div>
              <div className="rounded-md bg-gray-50 p-4 text-center">
                <div className="text-2xl font-bold text-amber-600">{words.filter((word) => !word.found).length}</div>
            ...
            <div className="flex w-full max-w-md gap-4">
              ...
              <Button className="game-button flex-1 gap-2" onClick={handleFinish}>
