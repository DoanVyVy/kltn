			...
			},
			{
				onSuccess: (data: any) => {
					if (data && data.grammars && data.grammars.length > 0) {
						...
						setLoading(false);
						toast.error(
							"Không có ngữ pháp nào trong danh sách ôn tập"
						);
					}
				},
				onError: (error) => {
	...
	const generateAnswerOptions = () => {
		...
		while (otherOptions.length < 3 && filteredGrammars.length > 0) {
			const randomIndex = Math.floor(
				Math.random() * filteredGrammars.length
			);
			if (questionType === 0) {
				...
				if (
					...
					const randomExampleIndex = Math.floor(
						Math.random() *
							filteredGrammars[randomIndex].examples!.length
					);
					otherOptions.push(
						filteredGrammars[randomIndex].examples![
							randomExampleIndex
		...
		if (correct) {
			dispatchAppEvent({
				...
				payload: {
					...
					categoryId: currentGrammar?.categoryId,
				},
				timestamp: new Date(),
			});
		}
				...
				<div className="container mx-auto px-4 py-8">
					<div className="flex justify-center flex-col items-center py-20 text-center">
						...
						<p className="text-gray-500 mb-6 max-w-md">
							Bạn chưa thêm ngữ pháp nào vào danh sách ôn tập hoặc
							đã ôn tập hết tất cả các quy tắc.
						</p>
						<Button
				...
				>
                    ...
                    `}
									...
									>
										...
										{showAnswer &&
											(questionType === 0
												? currentGrammar?.examples?.includes(
														option
												  )
												: option ===
												  currentGrammar?.title) && (
