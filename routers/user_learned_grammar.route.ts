import { getCurrentUser, getCurrentUserId } from "@/lib/server-auth";
	...
	getLearnedGrammar: baseProcedure
		...
		.query(async ({ ctx, input }) => {
			try {
				...
				const learnedGrammarIds =
					await prisma.userLearningAnswer.groupBy({
						by: ["grammarId"],
						where: {
							userId: user,
							isCorrect: true,
					...
					where: {
						contentId: {
							in: learnedGrammarIds.map(
								(g: UserLearningAnswerGroupByOutputItem) =>
									g.grammarId!
							),
						},
					},
				...
				const grammarStats = await Promise.all(
					grammars.map(async (grammar) => {
						const correctCount =
							await prisma.userLearningAnswer.count({
								where: {
									userId: user,
						...
						const lastAnswered =
							await prisma.userLearningAnswer.findFirst({
								where: {
									userId: user,
									grammarId: grammar.contentId,
								},
