    ...
    idioms: [
      {
        idiom: "Break a leg",
        meaning: "Good luck",
        explanation: "Used to wish someone good luck, especially before a performance or presentation",
      },
      {
        idiom: "Piece of cake",
        meaning: "Very easy",
...
export default function GameDataManagement() {
  ...
  const handleAddGame = () => {
    const newGame = {
      ...
      ...(formData.type === "sentence-scramble" && { sentences: formData.sentences }),
      ...(formData.type === "word-association" && { wordPairs: formData.wordPairs }),
  ...
  const openEditDialog = (game: any) => {
    ...
    setFormData({
      ...
      sentences: game.type === "sentence-scramble" ? [...game.sentences] : [{ original: "", scrambled: [""] }],
      wordPairs: game.type === "word-association" ? [...game.wordPairs] : [{ word: "", association: "" }],
      idioms: game.type === "idiom-challenge" ? [...game.idioms] : [{ idiom: "", meaning: "", explanation: "" }],
    })
  ...
  const handleWordPairChange = (index: number, field: string, value: string) => {
    const newWordPairs = [...formData.wordPairs]
    newWordPairs[index] = {
      ...newWordPairs[index],
      [field]: value,
        ...
        <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
          ...
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              ...
              <DialogDescription>Điền thông tin chi tiết cho trò chơi mới</DialogDescription>
            </DialogHeader>
                ...
                <TabsContent value="word-guess">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={handleAddWord}>
                        Thêm từ
                      </Button>
                    </div>
                    ...
                    {formData.words.map((word, index) => (
                      <div key={index} className="flex items-center gap-2">
                        ...
                        {formData.words.length > 1 && (
                          <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveWord(index)}>
                            <Minus size={16} />
                          </Button>
                        )}
                      </div>
                ...
                <TabsContent value="sentence-scramble">
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={handleAddSentence}>
                        Thêm câu
                      </Button>
                    </div>
                    ...
                    {formData.sentences.map((sentence, index) => (
                      <div key={index} className="border p-4 rounded-md">
                        ...
                        <div className="grid gap-4">
                          <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor={`sentence-${index}`} className="text-right">
                              Câu gốc
                            </Label>
                            <Input
                              id={`sentence-${index}`}
                ...
                <TabsContent value="word-association">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={handleAddWordPair}>
                        Thêm cặp từ
                      </Button>
                    </div>
                    ...
                    {formData.wordPairs.map((pair, index) => (
                      <div key={index} className="border p-4 rounded-md">
                        ...
                        <div className="grid gap-4">
                          ...
                          <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor={`association-${index}`} className="text-right">
                              Từ liên kết
                            </Label>
                            <Input
                              id={`association-${index}`}
                              value={pair.association}
                              onChange={(e) => handleWordPairChange(index, "association", e.target.value)}
                              className="col-span-3"
                ...
                <TabsContent value="idiom-challenge">
                  <div className="space-y-6">
                    <div className="flex justify-between items-center">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={handleAddIdiom}>
                        Thêm thành ngữ
                      </Button>
                    </div>
                      ...
                      <div key={index} className="border p-4 rounded-md">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          {formData.idioms.length > 1 && (
                            <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveIdiom(index)}>
                              <Minus size={16} />
                            </Button>
                          )}
                        </div>
                        <div className="grid gap-4">
                          <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor={`idiom-${index}`} className="text-right">
                              Thành ngữ
                            </Label>
                            <Input
                              id={`idiom-${index}`}
                              value={idiom.idiom}
                              onChange={(e) => handleIdiomChange(index, "idiom", e.target.value)}
                              className="col-span-3"
                              placeholder="Nhập thành ngữ"
                            />
                          </div>
                          <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor={`meaning-${index}`} className="text-right">
                              Ý nghĩa
                            </Label>
                            <Input
                              id={`meaning-${index}`}
                              value={idiom.meaning}
                              onChange={(e) => handleIdiomChange(index, "meaning", e.target.value)}
                              className="col-span-3"
                              placeholder="Ý nghĩa của thành ngữ"
                            />
                          </div>
                          <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor={`explanation-${index}`} className="text-right">
                              Giải thích
                            </Label>
                            <Textarea
                              id={`explanation-${index}`}
                              value={idiom.explanation}
                              onChange={(e) => handleIdiomChange(index, "explanation", e.target.value)}
                              className="col-span-3"
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAddGame} className="bg-game-primary hover:bg-game-primary/90">
                Thêm trò chơi
              </Button>
            </DialogFooter>
          </DialogContent>
                ...
                <TableRow key={game.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button variant="outline" size="icon" onClick={() => openDeleteDialog(game)}>
                        <Trash2 size={16} />
                      </Button>
                    </div>
                  </TableCell>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>Cập nhật thông tin cho trò chơi</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
            <Tabs
              value={selectedTab}
                  ...
                  {formData.sentences.map((sentence, index) => (
                    <div key={index} className="border p-4 rounded-md">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="font-medium">Câu {index + 1}</h4>
                        {formData.sentences.length > 1 && (
                          <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveSentence(index)}>
                            <Minus size={16} />
                          </Button>
                        )}
                      </div>
                      <div className="grid gap-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                          <Label htmlFor={`edit-sentence-${index}`} className="text-right">
                            Câu gốc
                          </Label>
                          <Input
                  ...
                  {formData.wordPairs.map((pair, index) => (
                    <div key={index} className="border p-4 rounded-md">
                      <div className="flex justify-between items-center mb-2">
                        <h4 className="font-medium">Cặp từ {index + 1}</h4>
                        {formData.wordPairs.length > 1 && (
                          <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveWordPair(index)}>
                            <Minus size={16} />
                          </Button>
                        )}
                      </div>
                      <div className="grid gap-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                          <Label htmlFor={`edit-word-${index}`} className="text-right">
                            Từ gợi ý
                          </Label>
                          <Input
                            id={`edit-word-${index}`}
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                          <Label htmlFor={`edit-association-${index}`} className="text-right">
                            Từ liên kết
                          </Label>
                          <Input
                ...
                <div className="space-y-6">
                  ...
                  {formData.idioms.map((idiom, index) => (
                    <div key={index} className="border p-4 rounded-md">
                      ...
                      <div className="grid gap-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                          <Label htmlFor={`edit-idiom-${index}`} className="text-right">
                            Thành ngữ
                          </Label>
                          <Input
                            id={`edit-idiom-${index}`}
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                          <Label htmlFor={`edit-meaning-${index}`} className="text-right">
                            Ý nghĩa
                          </Label>
                          <Input
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                          <Label htmlFor={`edit-explanation-${index}`} className="text-right">
                            Giải thích
                          </Label>
                          <Textarea
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEditGame} className="bg-game-primary hover:bg-game-primary/90">
              Lưu thay đổi
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Bạn có chắc chắn muốn xóa trò chơi "{currentGame?.title}"? Hành động này không thể hoàn tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
