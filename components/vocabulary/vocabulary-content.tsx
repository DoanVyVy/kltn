...
const CourseCardsSkeletonGrid = ({ count = 6 }) => (
...
const useVocabularyCounts = (categoryIds: number[]) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        const results = await Promise.all(
          categoryIds.map(async (categoryId: number) => {
            // Use the correct API call and properly access results
            const response = await utils.vocabularyWord.getWords.fetch({
              page: 1,
              limit: 1000,
              categoryId,
            });
            // Check if response has results property and it's an array
            const wordsCount = response.results ? response.results.length : 0;
            return { categoryId, count: wordsCount };
          })
        );

        setCounts(
          results.reduce(
            (acc: Record<number, number>, { categoryId, count }) => {
    ...
    fetchCounts();
  }, [categoryIdsKey, utils.vocabularyWord.getWords]);
...
export default function VocabularyContent() {
  ...
  const categoryIds = useMemo(
    () => allCourses.map((course: Course) => course.categoryId),
    [allCourses]
  );
  ...
  const learningCourseIds = useMemo(
    () =>
      (learningCourses as Progress[]).map(
        (progress: Progress) => progress.categoryId
      ),
    [learningCourses]
  );
  ...
  const learnedVocabularyCourses = useMemo(
    () =>
      (learningCourses as Progress[]).filter(
        (progress: Progress) =>
          progress.processPercentage > 0 &&
          progress.category !== null &&
          progress.category !== undefined &&
  ...
  const unexploredCourses = useMemo(
    () =>
      filteredCourses.filter(
        (course: Course) => !learningCourseIds.includes(course.categoryId)
      ),
  ...
  // Loading state for both tabs
  const isLoading = isLoadingLearning || isLoadingCourses || isLoadingCounts;

  // Content rendering with conditional logic for better error handling
  const renderContent = () => {
    if (isLoading) {
      return (
        <CourseCardsSkeletonGrid count={activeTab === "learned" ? 3 : 6} />
      );
    }
    ...
    if (activeTab === "learned") {
      ...
      return (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {learnedVocabularyCourses.map((progress: Progress) => (
            <CourseCard
    ...
    return (
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {unexploredCourses.map((course: Course) => (
          <CourseCard
