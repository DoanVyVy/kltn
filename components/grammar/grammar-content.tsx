...
const GrammarCardsSkeletonGrid = ({ count = 6 }) => (
...
export default function GrammarContent() {
  ...
  const learningCourseIds = useMemo(
    () =>
      (learningCourses as Progress[])
        .filter((progress: Progress) => progress.categoryId)
        .map((progress: Progress) => progress.categoryId!),
    [learningCourses]
  );
  ...
  const totalGrammarLearned = useMemo(
    () =>
      (learningCourses as Progress[])
        .filter((progress: Progress) => progress.category?.totalGrammar! > 0)
        .reduce(
  ...
  const renderTabContent = () => {
    if (isLoadingLearning || isLoadingCourses || isLoadingCounts) {
      return (
        <GrammarCardsSkeletonGrid count={activeTab === "learning" ? 3 : 6} />
      );
    }
    ...
    if (activeTab === "learning") {
      ...
      return (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          {learningGrammarCourses.map((progress: Progress) => (
    ...
    // Tab "explore"
    if (
      filteredCourses.filter((course: Course) => !course.isVocabularyCourse)
        .length === 0
    ) {
      return (
        <div className="flex flex-col items-center justify-center py-12 text-center">
    ...
    return (
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filteredCourses
          .filter((course: Course) => !course.isVocabularyCourse)
          .map((course: Course) => (
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (
                          learningGrammarCourses.length > 0 &&
                          learningGrammarCourses[0].categoryId
                        ) {
                          router.push(
                            `/grammar/practice/${learningGrammarCourses[0].categoryId}`
                          );
                        }
                      }}
                    >
